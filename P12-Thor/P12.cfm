	...
	<tr>
		<td>
			...
			<span class="TB-BODY">
				...
				It will be an <code>int</code> and we will simply call it <code>countGlb</code>.
			</span><br/>
			...
			<span class="TB-BODY">
				...
				will initialize the pins as <code>OUTPUT</code>s and set them to <code>LOW</code>. The
...
for(int i=2; i&lt;=7; i++){
  ...
  digitalWrite(i,LOW);
				...
				After setting the pins, all we need to do is set <code>countGlb</code> to 1 so that it 
				illuminates the first LED.
			</span><br/>
			<span class="TB-H2">The <code>loop()</code> Function</span><br/>
			<span class="TB-BODY"> <!-- THIS WILL NEED MAJOR LOOK OVER -->
				...
				We can figure out which LED to turn on by using <code>countGlb</code> in one of two ways;
				<ol>
					<li>Use <code>bitRead()</code> on <code>countGlb</code> to determine which bit is a one</li>
					<li>Use <code>bit()</code> and compare it to <code>countGlb</code></li>
				</ol>
				Using method one, we would use the following as the conditional in an if statement:
				<pre class="brush:mpide;">
bitRead(countGlb,i) == 1
</pre>
				This works because <code>i</code> starts at 0 and stops after 5, and <code>countGlb</code> will 
				be shifted to the left by 1 at the end of the <code>loop()</code> function, which means that there
...
bit(i) == countGlb
				...
				This works since it directly compares the value of <code>countGlb</code> to the value of 
				...
				<br/>
				So, by using method two, we can create the first part of the <code>loop()</code> function, which
  ...
  if(bit(i) == countGlb){
    ...
    if(i == 0){
      ...
      digitalWrite(i+1,LOW);
				...
				The last part of the <code>loop()</code> function simply deals with incrementing <code>countGlb</code>
				and delaying so we can see each individual light. We will use a bit-wise operator and modulo 
				operator to increment <code>countGlb</code>. We will need to use the modulo operator to ensure
				that <code>countGlb</code> does not go over the value that <code>bit(5)</code> since we need 
				<code>countGlb</code> to equal <code>bit(5)</code> and then loop back so that it equals 
				<code>bit(0)</code>. The following code does what we need done in the last part of the <code>loop()</code>
				function:
				<pre class="brush:mpide;">
delay(500);

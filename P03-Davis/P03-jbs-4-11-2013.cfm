			...
			<tr>
				<td>		
						...
						<span class="TB-BODY">
							...
							has a voltage level of 3.3V (and 0V when they are in a <code>LOW</code>
							state).  If we attach an LED between one of the output pins and a ground (GND)
							pin and then set the output pin to <code>HIGH</code>, the voltage that appears
							across the LED would exceed its maximum forward voltage. (As
							discussed <a href="P03files/P03Electric Review.cfm">elsewhere</a>, the
							nonlinear current-voltage (IV) relationship of the LED will cause too much
							current to flow, damaging the LED and possibly damaging the chipKIT board.) To
							prevent this, this we need to add a <i>current limiting resistor</i> to our
							circuit.  This resistor should be connected <i>in series</i> with the LED,
							meaning that any current that flows through the resistor must also flow through
							the LED.  A resistance of 220 &#8486; will ensure that the maximum forward
							voltage of the LED is not exceeded.  A discussion
							regarding <a href="P03files/P03Choosing Resistor.cfm">how to determine the size

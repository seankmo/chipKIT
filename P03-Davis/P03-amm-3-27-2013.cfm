...
<cf_Box Color="White" NoMargins="1">
  ...
  <table class="TBLAYOUT" style="width:100%;" cellpadding="0">
    <tr>
	  ...
	  <br/><br/>
	    ...
	    In <a href="P02.cfm">Project 02</a>,
	    we programmed the chipKIT microcontroller to control an onboard
	    LED. Now we will build our own external LED circuit and control it
	    with the microcontroller. To do so, we must understand the properties
	    of the circuit and its components.  If you are unfamiliar with the
	    fundamentals of electricity and simple electronic
	    components, <a href="P03files/P03Electric Review.cfm">a review of
	      these topics is available here</a>. 
	    <br/><br/>
      </td>    
    </tr>
  </table>
  ...
  <table class="TBLAYOUT" style="width:100%;" cellpadding="0">
    <tr>
      <td valign="top">		
			<br/><br/>
			<span class="TB-H1">Step 1: Planning the Circuit Design</span> 
			<br/><br/>	  
			<span class="TB-BODY">
			  ...
			  switch.  When the LED is illuminated, typically there is (and
			  ...
			  However, if the voltage drop exceeds the &ldquo;maximum forward voltage&rdquo; of
			  approximately 3 V, the LED might be damaged.  When they are in a
			  <code>HIGH</code> state, the output &ldquo;pins&rdquo; of the chipKIT boards has a voltage
			  level of 3.3 V (and 0 V when they are in a <code>LOW</code> state).  If we attach
			  an LED between one of the output pins and a ground (GND) pin and then
			  set the output pin to <code>HIGH</code>, the voltage that appears across the LED
			  ...
			  chipKIT board.) To prevent this, this we need to add a <i>current limiting
  ...
  <table class="TBLAYOUT" style="width:100%;" cellpadding="0">
	...
	<br/><br/>	  
        <span class="TB-BODY">
	  Having chosen our resistor, we can finally begin putting the circuit together. Grab a breadboard, 
	  ...
	  and chipKIT board both work. Figure 1 below how to connect different areas on a breadboard. 
	</span>
	<div align="center">
	...
	<br/><br/>
	<span class="TB-BODY">
	  The colors of each hole indicate which <i>rails</i> are electrically connected to each other. A group of 
	...
	<span class="TB-BODY">
	  ...
	  <i>5V pins</i> and <i>3.3V pins</i> all act as the positive terminals of the voltage
	  source. As their names suggest, the 5V and 3.3V pins suppyly the corresponding voltage.
	  The digital pins can be turned on and off by programming
	  them to <code>HIGH</code> or <code>LOW</code>, which we will discuss more thoroughly
	  later. When on <code>HIGH</code>, the digital pins output 3.3V. Now that we have a better understanding
	  of the how the chipKIT pins and the breadboard rails
	  work, we can begin wiring them together. For this example, use pin 3 as the positive
	  terminal (always denoted by a red wire) of the voltage source and ground pin for the negative
	  terminal (always denoted by a black wire). Attach the chipKIT board to the breadboard, as shown in Fig. 4.
	</span>
	<br/><br/>

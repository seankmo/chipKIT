		...
		<tr>
			<td>	    
							...
							<ul>
								...
								<li>Incorrect variable names:
									<ul>
										...
										<li> <code>bl@rg</code>  (at sign, and punctuation
				...
				<span class="TB-BODY"> 
					...
					Figure 2 illustrates the creation of a local variable called <code>ledPin</code>
					within the body of the <code>setup()</code> function.  Unfortunately, the programmer
					tried to reference the <code>ledPin</code> variable within the <code>loop()</code>
					...
					<ol>
						...
						<li>You can define two variables with the same name if they are in different
							...
							way because pin 6 is initialized in the <code>setup()</code> function but pin
							13 is used in the <code>loop()</code> function.
						...
						<li>When a variable is declared, one must specify the variable's type (for
							...
							aspects about the behavior of that variable.  The <code>const</code> qualifier
							...
							As an example of the use of the <code>const</code> qualifier, the following
							sets the integer variable <code>my_variable</code> to 56:

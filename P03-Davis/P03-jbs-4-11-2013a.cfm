<html>
			...
			<tr>
				<td>		
						...
						<span class="TB-BODY">
							...
							with a battery, a light switch, and a light bulb.  The chipKIT board will act
							as both the power source and the switch.  When the LED is illuminated,
							typically there is (and should be) a voltage drop across its terminals of
							approximately 2V.  The orientation we use when we place an LED in a circuit is
							important.  The LED will illuminate when the voltage drop is from
							the <a href="P03files/P03Electric Review.cfm#AnodeCathode">anode</a> to the
							<a href="P03files/P03Electric Review.cfm#AnodeCathode">cathode</a>.

							<br><br>

							
							It is possible to have a voltage drop across an LED of more than 2V.  However,
							if the voltage drop exceeds the &ldquo;maximum forward voltage&rdquo; of
							approximately 3V, the LED might be damaged.  When they are in a
							<code>HIGH</code> state, the output &ldquo;pins&rdquo; of the chipKIT boards
							have a voltage level of 3.3V (and 0V when they are in a <code>LOW</code>
							state).  If we attach an LED between one of the output pins and ground (0V),
							then set the output pin to <code>HIGH</code>, the voltage that appears across
							the LED will exceed its maximum forward voltage. 
							Because of the non-linear relationship between the current and the voltage in
							an LED, small changes in voltage can result in large changes in current.  This
							is discussed in greater depth
							<a href="P03files/P03Electric Review.cfm">here</a>.  Thus voltages that exceed
							the maximum formard voltage cause too much current to flow, damaging the LED
							and possibly damaging the chipKIT board.

							<br><br>

							To prevent this any damage, we need to add a <i>current limiting
							...
							resistor must also flow through the LED.  A resistance of 220 &#8486; will
							ensure that the maximum forward voltage of the LED is not exceeded.  A
							discussion regarding <a href="P03files/P03Choosing Resistor.cfm">how to
							determine the size of the current limiting resistor is available here</a>.
						</span>
					</td>
				</tr>
				...
				<tr>
					<td>
						...
						<br/><br/>
						<span class="TB-BODY">
							Having chosen our resistor, we can finally begin putting the circuit
							together. Grab a breadboard, an LED and the 220 &#8486; current limiting
							resistor. Any breadboard is fine. Before we wire the breadboard to the chipKIT
							board, it is important to understand how the connections for the breadboard and
							chipKIT board both work. Figure 1 below how to connect different areas on a
							breadboard.
						</span>
						<div align="center">
						...
						<br/><br/>
						<span class="TB-BODY">
							The colors of each hole indicate which <i>rails</i> are electrically connected
							to each other. A group of connected holes is called a <i>rail</i>. Rails are
							just a strips of metal that are used like a wire. In Fig. 2, you can see what
							all the rails would look like underneath the groups of holes.
						</span>
						<br/><br/>
						<div align="center">
						...
						<span class="TB-BODY">
							The <i>ground pin</i>, as seen in Fig. 3, represents the negative terminal of a voltage
							source. The <i>digital pins </i>,
  ...
  <table class="TBLAYOUT" style="width:100%;" cellpadding="0">
	...
	<span class="TB-BODY">
	  Now that we have the circuit properly wired, it is time to program the chipKIT board. Open MPIDE and start a new sketch. The 
	  code from Project 2 will work for this project. However, replace pin 13 with pin 3 in all the functions below.
	</span>								
      </td>
    </tr>
    <tr>

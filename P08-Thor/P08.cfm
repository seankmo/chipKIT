		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					<code>Sketch->Import Library</code>, and look towards the bottom of that menu for Bounce. 
					It should be under
...
Bounce javaBounce = Bounce(2, 50);
					...
					to <code>HIGH</code>. Previously (in Debouncing via Software), the LED state was used to determine if a
		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					access to them in both the <code>setup()</code> and <code>loop()</code> function. 
					They are <code>const</code> because the pins won't change during the program. 
...
Bounce bouncerGlb(btnPinGlb, debounceDelayGlb);
				...
				<span class="TB-BODY">
					The <code>setup()</code> function in this project is fairly straightforward. All we have to do is set
...
void setup()
{
  ...
  Serial.begin(9600);
				...
				<span class="TB-BODY">
					...
					&ldquo;<code>currentBtnState</code>.&rdquo; To get the current button state we use the 
					<code>read()</code> method on the variable <code>bouncerGlb</code>. We then have an if statement 
					where we use the <code>risingEdge()</code> mode on <code>bouncerGlb</code> to determine whether we increment

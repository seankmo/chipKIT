		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					Once the library is downloaded, unzip it. Then place the resulting folder in the libraries 
					folder of your sketchbook. If you do not know where this is, it can be found by going to
					<b>FILE-&gt;preferences</b> and looking at what the Sketchbook location. Alternatively, hitting
					<b>Ctrl+comma(,)</b> takes you to the same screen. Figure 2 should be the window that opens and the 
					sketchbook location will be at the top of the window.
					</td><td>
					...
					<b>Sketch->Import Library</b>, and look towards the bottom of the menu for Bounce. It should be under
					Contributed, which simply means that it was added by the user and not included by default. Figure 3
					is what you should see to determine if it worked. Also, this is the same process to import a library
					into a sketch.
					</td><td>
						<img src="P08files/importBounce.png"/>
						<div class="TB-FIGURES">Figure 3. Importing Bounce Library.</div>
					</tr></table>
					If you click the Bounce library in the import library menu, you should see 
					...
					returns a true if the pin had a rising edge during the last update.
					...
					that requires two inputs, a pin and the time to delay. Because of this, when we initialize the class we
					have to include the pin and debounce delay that we want to use. A direct example of what will work and 
					what won't may be helpful:
<div align="center">
...
Bounce javaBounce = Bounce(2, 50);
					...
					to <span class="TB-LITERAL1">HIGH</span>. Contrast this to project 7, where the LED state was used to determine if a
		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					are simply the pins for the button and the LED. They are also <span class="TB-BLACK">const</span> because
					...
					The final variable is actually a class object (a variable that is some class)
...
Bounce bouncerGlb(btnPinGlb, debounceDelayGlb);
				...
				<span class="TB-BODY">
					The <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> function is very simple in this project. All we have to do is set
...
void setup()
{
  ...
  Serial.begin(9600);
				...
				<span class="TB-BODY">
					...
					<span class="TB-BLACK">currentBtnState</span> and to get the current button we use the <span class="TB-KEYWORD2">read</span><span class="TB-BLACK">()</span>
					method on the variable <span class="TB-BLACK">bouncerGlb</span>. Then we have an if statement where we use
					the <span class="TB-BLACK">risingEdge</span><span class="TB-BLACK">()</span> mode on <span class="TB-BLACK">bouncerGlb</span> to determine if we increment

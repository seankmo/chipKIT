...
<body><cf_box color="white">
	...
	<table class="TBLAYOUT">
		<tr>
			<td>
				...
				<span class="TB-BODY">
					In <a href="P07.cfm">Debouncing via Software</a>, we introduced the concept of debouncing 
					a button was introduced, but we had to explicity program the debounce.
					In this project, the Bounce library will be used to debounce the
					input from a button. <br/>
					Just like in Debouncing via Software, this project will use a single button and LED, and it will count 
					the number of times the button has been pressed and display the result through the serial
					monitor. <br/>
					This project will use a library to debounce rather than manually coding or using a 
					hardware solution. <br/><br/>
				</span>
			</td>
		</tr>
	</table>
		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					but instead of the full breadboard,
					this project will use a mini breadboard and pins 7 and 8. Other than these minor differences
					the result should be the exact same as in Debouncing via Software, when the button is pressed the LED 
						...
						<tr>
							<td>
								<ol>
									...
									<li>Place a resistor (10 k&Omega;) on the same side button as the wire to 5V pin,
									...
									<li>Place another wire from ground (GND) to the other end of the 
										10 k&Omega; resistor</li>
									...
									resistor and the anode (long end) so that it is across the valley.</li>
		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					calling <span class="TB-KEYWORD2">digitalRead</span><span class="TB-BLACK">()</span>. Debouncing using a library is conceptually the
	...
	<table class="TBLAYOUT">
		<tr>
			<td>
				<cf_box color="putty">
				<span class="TB-H1">Step 1: Downloading the Bounce Library</span><br/><br/>
				<span class="TB-BODY">
					...
					Once the library has been downloaded, unzip the folder that was just downloaded. Then place 
					the resulting folder in the libraries 
					...
					which will hold the user installed &ldquo;Libraries,&rdquo; which will hold the user-insalled libraries.
					So, now that we have a location to install the bounce 
					library, copy the unzipped bounce folder and paste it into the newly created &ldquo;Libraries&rdquo; 
					folder. You 
					can check to see if the library was installed by starting (or restarting if already open) MPIDE, selecting
					<span class="TB-BLACK">Sketch->Import Library</span>, and look towards the bottom of that menu for Bounce. 
					It should be under
					&ldquo;Contributed&rdquo;, which simply means that it was added by the user and not included by default. Figure 3
					is the window you should see if the library was installed correctly. Also, this is the same process to import
					a library into a sketch, except that you would click on the library name to do so.
					<br/>
					...
					<span class="TB-BLACK">#include &lt;Bounce&gt;</span> at the top of your sketch. What this means is that when the
					...
					as easy as calling <span class="TB-KEYWORD2">digitalRead</span><span class="TB-BLACK">()</span>. The concept is the same as in project 7, but with the
					...
					<span class="TB-BODY">
						When you click on the Bounce library in the import library menu, you should see 
							<span class="TB-BLACK">#include &lt;Bounce&gt;</span> at the top of your sketch. This means that when
		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					<span class="TB-H2">The Constructor</span><br>
					<span class="TB-BODY">
					The Bounce class does not have a default constructor, which prevents initialization of a variable
					without any inputs. It only has a copy constructor and a constructor that requires two inputs, 
					a pin to debounce and the time to delay. Because of this, when we initialize the class we have
...
Bounce javaBounce = Bounce(2, 50);
					...
					<span class="TB-BODY">
					The update method is the most important method in this class, in addition to the constructor method. 
					This method contains the logic that is used to debounce the button or switch.
					...
					code at the beginning and end of the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function with the functional code
					in the middle of those two blocks of code. We can now just call the <span class="TB-BLACK">update</span><span class="TB-BLACK">()</span> 
					method and the rest of the code in the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function can be functional and
					...
					was called. This is presented in contrast to the code in <a href="P07.cfm">Debouncing via Software</a> 
					where two button state variables and two time state variables
					...
					<span class="TB-BODY">
					The risingEdge method is used to determine if the button's state is going from <span class="TB-LITERAL1">LOW</span>
					to <span class="TB-LITERAL1">HIGH</span>. Previously (in Debouncing via Software), the LED state was used to determine if a
					rising edge occured. The risingEdge method simply checks if the current state is <span class="TB-LITERAL1">HIGH</span>
		...
		<tr>
			<td>
				...
				<span class="TB-BODY">
					...
					are simply the pins for the button and the LED. We are making them global because we want
					access to them in both the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> and <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function. 
					They are <span class="TB-BLACK">const</span> because the pins won't change during the program. 
					We will call these variables &ldquo;btnPinGlb&rdquo; and &ldquo;ledPinGlb&rdquo; 
					where the PinGlb part simply means that the variable represents a pin and that 
					it is a global variable (all functions can use it).
					
					<br/><br/>
					
					The next variable is the amount to delay for the button debouncing. This variable is 
					<span class="TB-BLACK">const</span> <span class="TB-KEYWORD1">unsigned</span> <span class="TB-KEYWORD2">int</span>because we won't change the delay 
					in the debounce and a negative delay makes no sense. This variable will be called 
					debounceDelayGlb. See Debouncing via Software for more information on why we need a debounce delay.
					
					<br/><br/>
					
					The next variable is the count of the times the button has been pressed. It is an 
					<span class="TB-KEYWORD1">unsigned</span> <span class="TB-KEYWORD2">int</span> since there can't be negative button presses.
					The name of this variable will be countGlb because it is a global variable.
					The final variable is actually a class object (a variable that is a class), and
					that is the Bounce class. Due to how the Bounce class does
					not have a default constructor, it requires two inputs: the pin to debounce and the time to delay 
					for debouncing. We will just call the variable bouncerGlb because it is a global variable. However, to
					create this variable with the inputs it requires, we must treat it like we would a function. Observe the 
...
Bounce bouncerGlb(btnPinGlb, debounceDelayGlb);
					...
					to initialize it with the values of btnPinGlb and debounceDelayGlb. 
					<br/><br/>
				</span>
				
				<span class="TB-H2">The <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> Function</span><br/><br/>
				<span class="TB-BODY">
					The <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> function in this project is fairly straightforward. All we have to do is set
					the pin modes and start serial communications. The button will obviously be <span class="TB-LITERAL1">INPUT</span>
					and the LED will be <span class="TB-LITERAL1">OUTPUT</span>. Observe how we define the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span>
					function:
<div align="center">
        <cf_box color="white" style="width:95%; margin:8em 0 8em 0 ">
        <pre class="brush:mpide;">
void setup()
{
  ...
  Serial.begin(9600);
				...
				<span class="TB-H2">The <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> Function</span><br/><br/>
				<span class="TB-BODY">
					The first thing to do in the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function is to update the state
					of the button. This is done by simply using the <span class="TB-BLACK">update</span><span class="TB-BLACK">()</span> method on the variable
					<span class="TB-BLACK">bouncerGlb</span>. This will take care of the button debounce. The next thing is to
					create an integer and set it to the current button state. So we will call this variable
					&ldquo;<span class="TB-BLACK">currentBtnState</span>.&rdquo; To get the current button state we use the 
					<span class="TB-KEYWORD2">read</span><span class="TB-BLACK">()</span> method on the variable <span class="TB-BLACK">bouncerGlb</span>. We then have an if statement 
					where we use the <span class="TB-BLACK">risingEdge</span><span class="TB-BLACK">()</span> mode on <span class="TB-BLACK">bouncerGlb</span> to determine whether we increment
					<span class="TB-BLACK">countGlb</span> and sent the result to the computer. We then end the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span>
					function by writing the state of the button to the LED.<br/>
					The <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> should look something like this:
					<div align="center">
        <cf_box color="white" style="width:95%; margin:8em 0 8em 0 ">
        <pre class="brush:mpide;">
void loop()
{
  ...
  digitalWrite(ledPin, currentBtnState);
					...
					<span class="TB-BODY">
						...
						Now we have all of the information we need to debounce a button using the bounce library. The code
						for our sketch should look something like this:

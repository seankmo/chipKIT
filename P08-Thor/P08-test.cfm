			...
			<td>
				<cf_box color="putty">
					...
					<span class="TB-BODY">
						...
						be found by going to <span class="TB-BLACK">FILE-&gt;preferences</span> and looking at the Sketchbook
						...
						if already open) MPIDE, selecting <span class="TB-BLACK">Sketch->Import Library</span>, and looking
...
Bounce javaBounce = Bounce(2, 50);
					...
					using two blocks of code at the beginning and end of the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function
					with the functional code in the middle of those two blocks of code. We can now just
					call the <span class="TB-BLACK">update</span><span class="TB-BLACK">()</span> method and the rest of the code in
					the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function can be functional and focused on what you are trying
					...
					from <span class="TB-LITERAL1">LOW</span> to <span class="TB-LITERAL1">HIGH</span>. Previously (in Debouncing via Software),
					the LED state was used to determine if a rising edge occured. The risingEdge method
					simply checks if the current state is <span class="TB-LITERAL1">HIGH</span> and if there was a change
		  ...
		  <td>
			  <cf_box color="putty">
				...
				<span class="TB-BODY">
					...
					<span class="TB-BLACK">const</span> <span class="TB-KEYWORD1">unsigned</span> <span class="TB-KEYWORD2">int</span>, because we won't change
...
Bounce bouncerGlb(btnPinGlb, debounceDelayGlb);
				...
				<span class="TB-BODY">
					The <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> function in this project is fairly straightforward. All we
					have to do is set the pin modes and start serial communications. The button will
					obviously be <span class="TB-LITERAL1">INPUT</span> and the LED will be <span class="TB-LITERAL1">OUTPUT</span>. Observe how
					we define the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> function:
...
void setup()
{
  ...
  Serial.begin(9600);
				...
				<span class="TB-BODY">
					...
					<span class="TB-BLACK">bouncerGlb</span>. This will take care of the button debounce. The next thing
					is to create an integer and set it to the current button state. We will call this
					variable &ldquo<span class="TB-BLACK">currentBtnState</span>.&rdquo; To get the current
					button<!--I feel like a word is missing here. Do you mean button state?-->, we use
					the <span class="TB-KEYWORD2">read</span><span class="TB-BLACK">()</span> method on the variable <span class="TB-BLACK">bouncerGlb</span>. We then have
					an if statement where we use the <span class="TB-BLACK">risingEdge</span><span class="TB-BLACK">()</span> mode
					on <span class="TB-BLACK">bouncerGlb</span> to determine whether we increment

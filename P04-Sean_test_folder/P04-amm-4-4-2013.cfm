		...
		<cf_box color= "green" style="width:95%"> 
		  <table class="TBLAYOUT" style="width: 85%;" cellpadding="0">
			...
			<td>
			  ...
			  <br><br>
			  <span class="TB-BODY">
			    This project uses <code>if</code> statements and
			    the <code>digitalRead()</code> function.  Thus, it is important to provide a brief
			    explanation of these.
			    <br><br>

			    The <code>digitalRead()</code> function takes a single parameter, which is the pin
			    that is &ldquo;read.&rdquo; The function reads the electrical state of the pin,
			    and returns either <code>HIGH</code> or <code>LOW</code>, accordingly.  As an
			    example, assume we want to store the current state of pin 8 in the
			    variable <code>val</code>.  We would then use the following statement:
...
val = digitalRead(8);
			    ...
			    the <code>if</code> statement is not executed.  If we want an <code>if</code>
...
if (logical_expression) {
   // Code only executed if logical_expression is true.
			    ...
			    The details of what constitutes a logical expression are considered
			    below.
			    ...
			    an <code>else</code> clause with an <code>if</code> statement.  When present, the
			    code associated with the <code>else</code> clause will not be executed if the
			    logical expression is true, but will be executed if it is false.


			    A template for an <code>if</code> statement, that includes an <code>else</code>
   ...
   ... task2
			...
			<td>
			  <span class="TB-BODY">
			    ...
			    example a relational operators could be, &ldquo;<code>==</code>&rdquo; or
...
if ((digitalRead(BtnA) == true) &amp;&amp; (digitalRead(BtnB) == true)){
  digitalWrite(LedC, LOW);
			  ...
			  <span class="TB-BODY">
			    ...
			    ButtonB are
			    true, <code>digitalWrite()</code> LedC to
			    <code>HIGH</code>.&rdquo;

<!-- Based on \ChipKitDev\P04-Sean_test_folder\P04-jbs-5-14-2013.cfm 
     Posted by John, 5/16/2013.
-->
...
<html>
	...
	<table class="TBLAYOUT">
		<tr>
			<td>
				<span class="TB-H1">
					<span class="TB-KEYWORD2">digitalRead</span><span class="TB-BLACK">()</span> Function
				</span>
				<br><br>
				<span class="TB-BODY">
					...
					state of pin 7 in the integer variable <span class="TB-BLACK">val</span>.  We could accomplish this
...
val = digitalRead(7); // Assign the state of pin 7 to val.
					...
					<span class="TB-BLACK">val</span> is set to <span class="TB-LITERAL1">LOW</span>.  We can also set the value of the
					variable <span class="TB-BLACK">val</span> when we declare it as shown in the following statement:
...
if (logical_expression) {
   // Code only executed if logical_expression is true.
					...
					template for an <span class="TB-KEYWORD1">if</span> statement that includes an <span class="TB-KEYWORD1">else</span> clause
					is as follows:
...
if (logical_expression) {
   ...
   // Code only executed if logical_expression is false.
					...
					The logical expression that controls the actions of the <span class="TB-KEYWORD1">if</span> statement can
					...
					Examples of relational operators include <span class="TB-BLACK">&lt;</span> (less
					than), <span class="TB-BLACK">&gt;</span> (greater than), <span class="TB-BLACK">&gt;=</span> (greater than or equal
					to), &ldquo;<span class="TB-BLACK">==</span>&rdquo; (equal to), and
					<span class="TB-inequality">!=</span> (not equal to).  <i><b>A common programming error is to write a
					...
					the left of the equal sign).  Assuming <span class="TB-BLACK">x</span> is an integer variable, the
...
x &lt;= 5 // Is x less than or equal to five?
					...
					The first expression evaluates to true when the variable <span class="TB-BLACK">x</span> equals zero,
					and false for any other value.  The second expression evaluate to false
					if <span class="TB-BLACK">x</span> equals zero, and true for any other value.  The third expression
					evaluates to true if <span class="TB-BLACK">x</span> is less than or equal to five.  The
					...
					<table id="CT3">
						...
						<tbody>
							<tr>
								<td align="center">
									<span class="TB-BLACK">&lt;</span>
								</td>
								<td>Less Than</td>
								<td>
									<span class="TB-BLACK">7 &lt; x</span> <br>
									True if 7 is strictly less <span class="TB-BLACK">x</span>. <br>
									Said another way, true if <span class="TB-BLACK">x</span> is strictly greater than 7. <br>
									False is 7 is greater than of equal to <span class="TB-BLACK">x</span>.
								</td>
							</tr>
							<tr>
								<td align="center">
								...
								<td>
									<span class="TB-BLACK">x &lt;= 0</span> <br>
									True if <span class="TB-BLACK">x</span> is less than or equal to zero. <br>
									False if <span class="TB-BLACK">x</span> is greater than zero.
								</td>
							</tr>
							<tr>
								<td align="center">
								...
								<td>
									<span class="TB-BLACK">7 == x</span> <br>
									True if <span class="TB-BLACK">x</span> equals 7. <br>
									False if <span class="TB-BLACK">x</span> is not equal to 7.
								</td>
							</tr>
							<tr>
								<td align="center">
									<span class="TB-inequality">!=</span>
								</td>
								<td>Not Equal To</td>
								<td>
									<span class="TB-BLACK">x != 123</span> <br>
									True if <span class="TB-BLACK">x</span> is <i>not</i> equal to 123. <br>
									False if <span class="TB-BLACK">x</span> is equal to 123.
								</td>
							</tr>
							<tr>
								<td align="center">
								...
								<td>
									<span class="TB-BLACK">7 &gt;= x</span> <br>
									True if 7 is greater than or equal to <span class="TB-BLACK">x</span>. <br>
									False if 7 is strictly less than <span class="TB-BLACK">x</span>.
								</td>
							</tr>
							<tr>
								<td align="center">
								...
								<td>Greater Than</td>
								<td>
									<span class="TB-BLACK">x &gt; 97</span> <br>
									True if <span class="TB-BLACK">x</span> is strictly greater than 97. <br>
									False is <span class="TB-BLACK">x</span> is less than or equal to 97.
								</td>
							</tr>
						</tbody>
					</table>
					...
					expression&rdquo; that controls the behavior of an <span class="TB-KEYWORD1">if</span> statement.  With
					that in mind, an example of an <span class="TB-KEYWORD1">if</span> statement that we'll use to control
					one of the LEDs is:
...
if (digitalRead(btnA) == HIGH) {
  ...
  digitalWrite(ledA, LOW);
					...
					The first thing that happens when this <span class="TB-KEYWORD1">if</span> statement is executed, is the
					logical expression in the parentheses in line 1 is evaluated.  This logical expression
					has, on the left side of the comparison operator, the <span class="TB-KEYWORD2">digitalRead</span><span class="TB-BLACK">()</span>
					function.  This function is executed, where the assumption is that the
					parameter <span class="TB-BLACK">btnA</span> corresponds to the pin attached to button A.  The value
					this function returns is compared to the constant value <span class="TB-LITERAL1">HIGH</span>.  If the
					value read is <span class="TB-LITERAL1">HIGH</span>, line 2 is executed.  If the value is
					not <span class="TB-LITERAL1">HIGH</span> (i.e., if it is <span class="TB-LITERAL1">LOW</span>), then line 5 is executed.
					The assumption is that <span class="TB-BLACK">ledA</span> correspond to the pin to which LED A is

			...
			<ol>
			  <span class="TB-BODY">
			    ...
			    <li> Place three buttons into the breadboard as shown in Fig. 9. Note the
			      ...
			      secton of the board (with the board placed as shown in Fig. 9).&nbsp; Even if
			      your breadboard does not have the aforementioned valley, the circuit will still
			      be functional provided that you retain this button orientation. <br>
			    </li>
			    ...
			    <li>For each button, connect one end of a 10 k&#8486; resistor to the left side of
			      the button.&nbsp;</li>
			    <li>Connect the other end of each resistor
			      to the ground bus strip (you can use a
			      wire to connect from the resistor to the bus
			      strip, as shown in Fig. 9, or you can
			      directly connect the end of the resistor
			      to the bus strip).<br>
			    </li>
			    <li>When a button is pressed, it supplies 3.3V to the corresponding digital I/O
			      pin.&nbsp; The chipKIT Max32 and Uno32 boards are designed to recognize a range
			      of voltages&mdash;from a minimum of 2.4V to a maximum of 5.5V&mdash;as a
		...
		<table class="TBLAYOUT" style="width: 85%;" cellpadding="0">
		  <tbody>
			...
			<span class="TB-BODY">
			  ...
			  shown in Fig. 10.  The next step is to write the software that controls the
			  circuit.<br><br>
			</span>
		      </td>
		    </tr>
			...
			<div class="TB-FIGURES">
			  <span style="font-style: italic;">Fig. 10: Circuit showing all the buttons and LEDs.
			  </span>
			</div>
			<br><br>
		      </td>
		...
		<cf_box color= "green" style="width:95%"> 
		  <table class="TBLAYOUT" style="width: 85%;" cellpadding="0">
			...
			<td>
			  ...
			  <br><br>
			  <span class="TB-BODY">
			    This project uses <span class="TB-KEYWORD1">if</span> statements and
			    the <span class="TB-KEYWORD2">digitalRead</span><span class="TB-BLACK">()</span> function.  Thus, it is important to provide a brief
			    explanation of these.
			    <br><br>

			    The <span class="TB-KEYWORD2">digitalRead</span><span class="TB-BLACK">()</span> function takes a single parameter, which is the pin
			    that is &ldquo;read.&rdquo; The function reads the electrical state of the pin,
			    and returns either <span class="TB-LITERAL1">HIGH</span> or <span class="TB-LITERAL1">LOW</span>, accordingly.  As an
			    example, assume we want to store the current state of pin 8 in the
			    variable <span class="TB-BLACK">val</span>.  We would then use the following statement:
			    <br><br>
...
val = digitalRead(8);
			    ...
			    An <span class="TB-KEYWORD1">if</span> statement performs a test of a given logical expression.  If
			    this expression is &ldquo;true,&rdquo; then the code that follows
			    the <span class="TB-KEYWORD1">if</span> statement is executed.  However, if the logical expression is
			    <em>not</em> true (in other words, it is false), then the code following
			    the <span class="TB-KEYWORD1">if</span> statement is not executed.  If we want an <span class="TB-KEYWORD1">if</span>
			    statement to control the execution of multiple lines of code this code must be
			    enclosed in braces.  The following is a &ldquo;template&rdquo; for
...
if (logical_expression) {
   // Code only executed if logical_expression is true.
			    ...
			    The details of what constitutes a &ldquo;logical expression&rdquo; are considered
			    below.
			    ...
			    an <span class="TB-KEYWORD1">else</span> clause with an <span class="TB-KEYWORD1">if</span> statement.  When present, the
			    code associated with the <span class="TB-KEYWORD1">else</span> clause will not be executed if the
			    logical expression is true, but will be executed if it is false.


			    A template for an <span class="TB-KEYWORD1">if</span> statement, that includes an <span class="TB-KEYWORD1">else</span>
   ...
   ... task2
			...
			<td>
			  <span class="TB-BODY">
			    ...
			    here we will restrict ourselves to comparision expression.  A comparison
			    expression is much like a mathematical equation except that it evaluates only to
			    true or false.  Like mathematical expressions, which have their own set of
			    operators (i.e., <code>+</code> , </code>-, <Arithmetic>/</Arithmetic>
			    and <code>*</code>) , comparison expressions have their own set as well. For
			    example a relational operators could be, &ldquo;<span class="TB-BLACK">==</span>&rdquo; or
			    ...
			    expression assuming x is an int type variable, could be:
			    <ol>
			      <li>

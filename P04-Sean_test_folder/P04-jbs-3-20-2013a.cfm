		  ...
		  <table class="TBLAYOUT" style="width: 85%;" cellpadding="0">
			...
			<td colspan="1">
			  <span class="TB-BODY">
			      ...
			      to interpret what we're seeing. Figure 6
			      shows a button circuit with
			      a <i>pull-down resistor</i> when the
			      button is open (so no current can flow).
			      A circuit that provides no path for
			      current to flow is known as
			      an <a href="P04_KVL_KCL.html#Open"><i>open</i></a>
			      circuit.  When two points in a circuit
			      are connected, with no resistance
			      between them, these points are said to
			      be <i>shorted</i> together.  We
			      sometimes simply say that there is a
			...
			<ol>
			  <span class="TB-BODY">
			    ...
			    <li>When each button is pressed, it will
			      ...
			      <code>HIGH</code> input for digital I/O pins.  That is,
			      when the software function <code>digitalRead()</code>
			      is called, it will return a <code>HIGH</code> if the
			      voltage on the pin being read from is
			      within this range.  The <code>digitalRead()</code>
		  ...
		  <table class="TBLAYOUT" style="width: 85%;" cellpadding="0">
			...
			<td>
			  ...
			  <span class="TB-BODY">
			    ...
			    pin, and output either a <code>HIGH</code> or <code>LOW</code>
...
val = digitalRead(8);
			    ...
			    returned by the <code>digitalRead()</code>
			    ...
			    apply.  The syntax for an <code>if</code>
			    statement, that includes
			    an <code>else</code> clause, is as
			    follows:

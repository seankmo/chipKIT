	  ...
	  <tr>
		  <td>
			  ...
			  <span class="TB-BODY">
				  ...
				  button outputs a <span class="TB-LITERAL1">LOW</span> level when the button is not pushed and
				  a <span class="TB-LITERAL1">HIGH</span> level when the it is pushed.  Refer
			  ...
			  <span class="TB-BODY">
				  ...
				  port, <span class="TB-KEYWORD3">Serial</span>, does double duty in that it both communicates through the
				  USB as well as through pins 0 and 1.  The keywords that are used in a sketch are
				  actually variables that belong to either <span class="TB-BLACK">HardwareSerial</span>
				  or <span class="TB-BLACK">USBSerial</span>. <!--Do these belong in code tags? Are they libraries?-->
				  ...
				  The first and most important function in <span class="TB-KEYWORD3">Serial</span> is
				  the <span class="TB-BLACK">Serial.begin</span><span class="TB-BLACK">(speed)</span> method. This method configures the hardware for
				  ...
				  The next two methods are essentially the same. The <span class="TB-BLACK">Serial.print</span><span class="TB-BLACK">(message)</span>
				  and the <span class="TB-BLACK">Serial.println</span><span class="TB-BLACK">(message)</span> methods both send a message, but
				  ...
				  sending just one of the two repeatedly. Note that <span class="TB-KEYWORD2">print</span><span class="TB-BLACK">()</span> is on one line
				  with no space between the messages and that <span class="TB-KEYWORD2">println</span><span class="TB-BLACK">()</span> has the message on
			  ...
			  <span class="TB-BODY">
				  ...
				  We need a variable to store the state of the LED (whether it is <span class="TB-LITERAL1">HIGH</span>
				  or <span class="TB-LITERAL1">LOW</span>). This is because when the button is pressed, the LED should
				  toggle to its other state- from <span class="TB-LITERAL1">LOW</span> to <span class="TB-LITERAL1">HIGH</span>, or vice
				  versa. This variable should be an integer <!--Why?-->.  We'll call
				  it <span class="TB-BLACK">stateLedGlb</span>. Since this is a state variable for the LED, this variable
				  ...
				  state. Let's call these variables <span class="TB-BLACK">buttonCurrentGlb</span>
				  and <span class="TB-BLACK">buttonLastGlb</span>, respectively, since we are interested in both the
				  current and last states of the button. Both of these variables will be an integer
				  value because the button will be either <span class="TB-LITERAL1">HIGH</span> or <span class="TB-LITERAL1">LOW</span>.
				  ...
				  called <span class="TB-BLACK">timeCurrentGlb</span> and <span class="TB-BLACK">timeLastGlb</span>. Both of these
				  variables should be integers since the <span class="TB-KEYWORD2">millis</span><span class="TB-BLACK">()</span> function returns an
				  integer.
				  ...
				  variable <span class="TB-BLACK">secondsGlb</span>.
				  ...
				  straightforward. Place <span class="TB-BLACK"></span><span class="TB-BLACK">(float)</span> in front of the number, variable, or
...
seconds = (float) (timeCurrentGlb - timeLastGlb) / 1000;
				  ...
				  these variables <span class="TB-BLACK">ledGlb</span> and <span class="TB-BLACK">btnGlb</span> respectively.
		  ...
		  <td>
			  <cf_box color="putty">
				  ...
				  <span class="TB-BODY">
					  ...
					  However, before we can define the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> and <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span>
...
const int btnGlb = 4;
					  ...
					  <span class="TB-H2">Step 2: Using the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> Function</span>

					  <br><br>

					  Now that the variables are taken care of, we can focus on how to use them in
					  the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span> function. First, we should start the serial communications
					  by calling <span class="TB-BLACK">Serial.begin</span><span class="TB-BLACK">()</span>. The next step is to set the mode of the
					  pins we are using and to set the LED to the current value of the
					  variable <span class="TB-BLACK">stateLedGlb</span>. The final step in the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span>
					  function is to initialize the &ldquo;last&rdquo; variables for time and button
					  state so that the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function doesn't try to compare to
					  uninitialize variables which can have an unknown value. <!--Please clarify the
					  previous sentence. Doesn't make much sense-->Thus, the <span class="TB-KEYWORD3">setup</span><span class="TB-BLACK">()</span>
...
void setup()
{
  ...
  buttonLastGlb = digitalRead(btnGlb);
					  ...
					  <span class="TB-H2">Step 3: Using the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> Function</span>

					  <br><br>

					  Now we need to tackle the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function.  This requires that we only
					  send a message when the pin reading the state of the button sees a change and that
					  the change is from <span class="TB-LITERAL1">LOW</span> to <span class="TB-LITERAL1">HIGH</span>.  We can do this by using
					  ...
					  simply assigning the value of a <span class="TB-KEYWORD2">digitalRead</span><span class="TB-BLACK">()</span> to
					  the <span class="TB-BLACK">buttonCurrentGlb</span>.  Then, in the conditional statement, we check to
					  see if the <span class="TB-BLACK">buttonCurrentGlb</span> is <span class="TB-LITERAL1">HIGH</span> and if it is
					  different from <span class="TB-BLACK">buttonLastGlb</span>.  This conditional statement should look
					  like the following:
        ...
        <pre class="brush: mpide;">
		    ...
		    // Message code in here...
					  ...
					  do is to find the current time by calling <span class="TB-KEYWORD2">millis</span><span class="TB-BLACK">()</span> and
					  assigning <span class="TB-BLACK">timeCurrentGlb</span> to it. Now that we have the current time and
        ...
        <pre class="brush: mpide;">
		    ...
		    stateLedGlb = !stateLedGlb; 
					  ...
					  pressed to &ldquo;now.&rdquo; Also, right before the end of the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span>
					  function, we will set the last button state to the current one.  So the
					  final <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function should look like:
			  ...
			  <table id="CT3">
				  ...
				  <tbody>
					  ...
					  <tr>
						  ...
						  <td width="80%">
							  <ul>
								  <li><span class="TB-BLACK">Serial.begin</span><span class="TB-BLACK">()</span></li>
								  <li><span class="TB-BLACK">Serial.print</span><span class="TB-BLACK">()</span></li>
								  <li><span class="TB-BLACK">Serial.println</span><span class="TB-BLACK">()</span></li>
								  <li><span class="TB-KEYWORD2">millis</span><span class="TB-BLACK">()</span></li>
								  <li><span class="TB-KEYWORD2">micros</span><span class="TB-BLACK">()</span></li>
							  </ul>
						  </td>
					  </tr>
				  </tbody>

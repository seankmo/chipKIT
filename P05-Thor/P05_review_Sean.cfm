		...
		<tr>
			<td width="50%" align="center">
				<span class="TB-PROJECTTITLE" align="center">
					...
					<font color="red"> maybe shorten the title to just "Project 5: Button Controlled LED with Serial Output", just a thought</font>
				</span>
			</td>
		</tr></td></tr>
	</table></cf_box>
	...
	<table class="TBLAYOUT" style="width:100%;" cellpadding="0">
		<tr>
			<td valign="top">
				<p>
					...
					<span class="TB-BODY">
						...
						<font color="red">" maybe give a more in depth description of what the project is, instead of "this project builds off what was introduced in project 4, ",
						it kind of sounds a bit vague, maybe say what was introduced in project 4,   also mentioning the title of the project as well might make
						things more clear. ie "Project 5: Button Controlled LED with Serial Output" 
						really just a more detailed description might be nice. </font>
			...
			<td width="320" align="center">
				...
				<div class="TB-FIGURES"><font color="red">Fig 1. MPIDE Serial Monitor.</font></div>
			</td>
		</tr>
	</table>
			...
			<td>
				...
				<table border="1" cellpadding="0" cellspacing="0" style="text-align:center;">
					...
					<tr>
						...
						<td>
							1 <font color="red"> What kind, Ohm value </font>
						</td>
						<td align="center">
	...
	<table class="TBLAYOUT" cellpadding="0">
		<tr>
			<td>
				...
				<span class="TB-BODY">
					This circuit will require a <font color="red"> again the value </font> resistor, a button, and three wires.
					
					<font color="red"> Saying "A pull down resistor" implies that there is another one, in addition to what you mentioned above, you could say
					"the 10 kOhm resistor will be used as a pull down resistor, so that the button outputs a LOW level when not pressed, and HIGH when pressed."  --or some variation on that idea
					You could also mention to reference Project 4 for a more in depth explanation of buttons and pull-down resistors (instead of just , "this use of a button was introduced"</font>
					...
					pull-down resistor.
					<br/>
					To set the circuit up, start by placing the button onto the <font color="red">breadboard</font>, 
					bridging the gap in the middle of <font color="red">the breadboard</font>. Just like in Figure 2, 
					place <font color="red">the</font> resistor on one of the legs of the button <font color="red">(maybe a comment saying, remember they are electrically connected)</font> and connect the other 
					end of <font color="red">the</font>resistor to the ground pin of the chipKIT board <font color="red"> by</font> using another wire. 
					...
					the leg of the button directly across from the resistor. <font color="red"> this is somewhat hard to follow </font>
				</span>
				<font color="red"> Maybe organize the instruction paragraph into a list, it will be easier to follow </font>
				
			</td>
			<td align="center">
				<img src="P05files/P05_bbs.png" height="250"/>
				<div class="TB-FIGURES"><font color="red">Fig 2. Circuit Setup.</font></div>
			</td>
		</tr>
	</table>
	<table class="TBLAYOUT" style="width:100%;" cellpadding="0">
			...
			<td valign="top">
				<p>
					...
					<span class="TB-BODY">
						...
						<font color="red">I think an explanation of class methods may be in order here.  Serial.begin() is not just a function.
						...
						any kind of depth about that just let the reader know that not all serial connections are the same.</font>
						...
						ends of the communication or errors will occur and cause all sorts of problems.<font color="red"> you could probably say something along the
						lines of "will cause a failure in communication" as opposed to all sorts of problems,  -- just being more specific --
						</font><br/><br/>
						
						The most common baud rate is 9600. The MPIDE Serial Monitor can only support <font color="red">the</font> following 
						baud rates: 300, 1200, 2400, 4800, 9600, 14400, 19200, 28800, 38400, 57600, <font color="red">and</font> 115200. 
				...
				<table>
					<tr>
						<td>
							...
							<div class="TB-FIGURES"><font color="red">Fig 2a. Looped Serial.print().</font></div>
						</td>
					</tr>
					<tr>
						<td>
							<img src="P05files/helloln.png" height="200"/>
							<div class="TB-FIGURES"><font color="red">Fig 2b. Looped Serial.println().</font></div>
						</td>
					</tr>
				</table>
			</td>
			...
			<td>
				<p>
					...
					<span class="TB-BODY"><br/><br/>
						...
						and let it be called <code>LED_state</code>  <font color="red">check John's new naming convention in the style guide</font>.<br/>
		...
		<tr>
			<td>
				<p>
					...
					<span class="TB-BODY"><br/><br/>
						<font color="red"> maybe consider breaking this up this whole section into sub segments such as: variable initialization,  setup, and loop,  so that it is easier to read</font>
...
<cf_box color="white"
	...
	<font color="red"> maybe consider rearranging things like this,  it is easier to read </font>
<pre class="brush:mpide;">

int LED_state = LOW;  // The LED state, initially set to LOW

int curr_btn_state;   // Button states
int last_btn_state;

int curr_time; 	      // Time variables
int last_time;

float seconds;	      // Difference in time in seconds
</pre></cf_box></center>
...
void setup()
{
  ...
  pinMode(4,INPUT);			// Set the pins
  pinMode(13,OUTPUT);
  
  digitalWrite(13,LED_state);
  
  last_btn_state = digitalRead(4);	// Initialize state variables
  last_time = millis();
}
</pre></cf_box></center>
...
if(HIGH == curr_btn_state && curr_btn_state != last_btn_state){
  // Message code in here...
						...
						thing to do is to find the current time by calling <kw2>millis</kw2><tt>()</tt> <font color="red"> explain millis???</font>and 
...
curr_time = millis(); 				 // Get the time

seconds = (float) (curr_time - last_time)/1000;  // Find difference and convert to seconds

/* Toggle the LED
* Remember that ! is a not operation when applied to a variable,
* it returns the inverse (true to false etc...)
*/
								...
								<table border="1" cellpadding="0" cellspacing="0">
									...
									<tr>
										...
										<td>
											...
											turned on <font color="red"> <----- since the sketch starts, i think.  if you reset the sketch then it resets too , i belive </font>, or the time since the clock that millis uses 
									...
									<tr>
										...
										<td>
											...
											This function is also more precise than millis, but also less accurate. <font color="red"> <----- what do you mean by this </font>
										</td>
									</tr>
								</table>
							</td>

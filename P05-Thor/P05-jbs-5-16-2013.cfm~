...
<cf_Box Color="White">
	  ...
	  <tr>
		  <td>
			  ...
			  <span class="TB-BODY">
				  ...
				  this project, we will use a button to control an on-board LED and send a message
			  ...
			  <div align="center">
				  ...
				  <tbody>
					  <tr>
						  ...
						  <td align="center" class="image"><img src="Parts/PushButton.svg"></td>
					  </tr>
					  <tr>
						  <td class="qty">1</td>
						  <td>10 k&Omega; resistor</td>
						  <td align="center" class="image"><img src="Parts/resistor_10K.svg"></td>
					  </tr>
				  </table>
  ...
  <table class="TBLAYOUT">
	  <tr>
		  <td>
			  ...
			  <br><br>
			  <cf_imagebox align="right" path="P05files/P05_bbs.svg" width="279px" caption="Fig. 1. Initial circuit configuration.">
			  <span class="TB-BODY">
				  ...
				  button outputs a <code>LOW</code> level normally and a <code>HIGH</code> level when
				  the button is pressed. Refer to <a href="../P04/P04.cfm">Button-Controlled LEDs</a> for a more
				  in-depth explanation of buttons and pull-down resistors. 
				  <br><br>

				  <ol>
					  <li>Place the button so that it bridges the valley in the breadboard.<br></li>
					  <li>Then place the 10 k&Omega; resistor so that it connects to one of 
						  the legs of the button.<br></li>
					  <li>Use a wire to connect the other end of the resistor to a ground pin
						  on the chipKIT board (shown as the black wire in Fig. 1).<br></li>
					  <li>Use a wire to connect pin 4 to the leg of the button directly across 
						  ...
						  connected to ground when the button is not pressed.<br></li>
			 ...
			 <span class="TB-BODY">
				 ...
				 <br><br>
				 <cf_imagebox align="right" path="P05files/hello.png" width="309" caption="Fig. 2. Looped Serial.print().">
				 ...
				 print() only sends the message and nothing else. See Figures 2 and 3 to see 
				 ...
				 <cf_imagebox align="right" path="P05files/helloln.png" width="309" caption="Fig. 3. Looped Serial.println().">
			 </span>
			 ...
			 <span class="TB-BODY">
				 ...
				 or <code>LOW</code>). This is because when the button is pressed, the LED should toggle
				 to its other state- from <code>LOW</code> to <code>HIGH</code>, or vice versa. This
				 variable should be an integer <!--Why?-->.  We'll call
				 ...
				 current and last states of the button. Both of these variables will be an integer value
				 because the button will be either <code>HIGH</code> or <code>LOW</code>.
				 ...
				 states.  However, instead of just noting that a difference is present, the value of the
				 difference is relevant.  Let these two varibles be called <code>timeCurrentGlb</code>
				 and <code>timeLastGlb</code>. Both of these variables should be integers since
				 the <code>millis()</code> function returns an integer.

				 <br><br>
				 
				 Finally, if you wish to convert the time from milliseconds to seconds, another variable
				 can help us keep do this simply. Since the conversion will have fractional parts, this
				 variable should be a <i>float</i>, or floating point number. This is a number that can have a
				 decimal point in it.  Lets call this variable <code>secondsGlb</code>.
				 ...
				 operation you would like to convert. Thus, to convert milliseconds to seconds, we'd use
				 a statement like this one:<br>

				 <pre>seconds = (float) (timeCurrentGlb - timeLastGlb) / 1000;</pre>

				 We also have constant variables for the LED pin and the button pin. We will just
  ...
  <table class="TBLAYOUT">
	  <tr>
		  <td>
		    ...
		    const int btnGlb = 4;
					  ...
					  <span class="TB-BODY">
						  ...
						  the mode of the pins we are using and to set the LED to the current value of the
						  variable <code>stateLEDGlb</code>. The final step in the <code>setup()</code>
						  function is to initialize the &ldquo;last&rdquo; variables for time and button state so that
						  the <code>loop()</code> function doesn't try to compare to uninitialize
						  variables which can have an unknown value. <!--Please clarify the previous
						  sentence. Doesn't make much sense-->Thus, the <code>setup()</code> function
		    ...
		    timeLastGlb = millis();
					  ...
					  <span class="TB-BODY">
						  ...
						  we only send a message when the pin reading the state of the button sees a
						  change and that the change is from <code>LOW</code> to <code>HIGH</code>. We can
						  do this by using the conditional statement introduced in the last
						  project. Before the using conditional statement, we need to read the current state of
						  the button, which is simply assigning the value of a
						  <code>digitalRead()</code> to the <code>buttonCurrentGlb</code>. Then, in the
						  conditional statement, we check to see if the <code>buttonCurrentGlb</code>
						  is <code>HIGH</code> and if it is different
						  from <code>buttonLastGlb</code>. This conditional statement should look like the
						  following:
		...
		Before we can use the message code we figured out earlier, we must find the time difference
		and convert it into seconds, and then toggle the LED. The first thing to do is to find the
		current time by calling <code>millis()</code> and assigning <code>timeCurrentGlb</code> to
		it. Now that we have the current time and the last time, we can find the difference in time
		and convert it to seconds. The last thing to do is toggle the LED, simply assigning
		it to its opposite <!--to its opposite what? state?-->.  The code fragment to do is shown
		here:
		  ...
		  Since we already have the messaging code, we can now talk about the subsequent code We will
		  now write the new state to the LED and set the last time the button was pressed to
		  &ldquo;now.&rdquo; Also, right before the end of the <code>loop()</code> function, we will
		  set the last button state to the current one.  So the final <code>loop()</code> function
		  should look like:
		  ...
		  <td>
			  ...
			  <tbody>
				  ...
				  <tr>
					  ...
					  <td width="80%">
						  <ul>
						  <li><span class="TB-KEYWORD2">Serial.begin</span><tt>()</tt></li>
						  <li><span class="TB-KEYWORD2">Serial.print</span><tt>()</tt></li>
						  <li><span class="TB-KEYWORD2">Serial.println</span><tt>()</tt></li>
						  <li><span class="TB-KEYWORD2">millis</span><tt>()</tt></li>
						  <li><span class="TB-KEYWORD2">micros</span><tt>()</tt></li>
						  </ul>

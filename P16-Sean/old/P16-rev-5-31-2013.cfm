    ...
    <table class="TBLAYOUT">
		<tr>
	    	<td>
				...
				You are probably familiar with the <code>analogWrite()</code> function from the &ldquo;Changing Color of Tricolor LED&rdquo; or &ldquo;Breathing LED&rdquo; projects.  The function simply transmits a PWM signal, where the input parameters specify the output pin and the duty cycle of the signal. 
  
				<br><br>
    
				However, you may not be familiar with the <code>analogRead()</code> function. As its name suggests, this function will convert a signal received on one of the analog pins (analog pins on chipKIT boards have labels preceded by an &ldquo;A&rdquo; to distinguish them from digital pins) to signal that is readable as a numerical value. 

	        	<br><br>
     
				ChipKIT boards use 10-bit analog-to-digital converters (ADCs), so analog values will be approximated to one of 1024 values (The number of values an ADC can distinguish between is equal to 2<sup>N</sup>, where N is the number of bits of the ADC). This type of approximation is known as signal &ldquo;quantization.&rdquo; When a continuous signal (like an analog signal) is &ldquo;quantized,&rdquo; it only means that its value is restricted to a discrete number of levels. For example, if you have an analog signal that ranges from any value between 0V and 8V, and you have 8 discrete levels. An analog signal with a level of 6.2V would map to 6 (the closest discrete level). 

	        	<br><br>
     
				The <code>analogRead()</code> function assigns the number 1024 to the max reference voltage (and likewise 0 for 0V). The max reference voltage is a value set with the <code>analogReference()</code> function. You will specify <code>DEFAULT</code> as the parameter most frequently when calling this function. On chipKIT boards this designates the max reference voltage as 3.3V. 
     
				<br><br>
     
				Sometimes, it may be useful to use the <code>EXTERNAL</code> parameter for <code>analogReference()</code>. This specifies the reference voltage to be taken from a physical pin rather than an internal reference (On the chipKIT Max32, this is pin 44, but this will change from chipKIT board to chipKIT board. The exact pin can be found in your board's reference manual.).
				</span>
      	  	</td>
	  	</tr>
    </table>
    ...
    <table class="TBLAYOUT">
	  	<tr>
	  		<td>
	 	 		...
	 	 		<span class="TB-BODY">
	    		Once you have an understanding of all of the functions in this sketch, you will find that it is very simple and straightforward. The <code>setup()</code> portion of the sketch simply sets the ADC reference value to <code>DEFAULT</code>, which is 3.3V, and sets the <code>pinMode</code> of the LED to <code>OUTPUT</code>. Note that you don’t have to call <code>pinMode</code> for analog input pins (unless you are going to use them as a digital pins). 

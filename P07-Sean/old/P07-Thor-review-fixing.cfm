...
<br>
    ...
    <div class="TB-FIGURES">Fig 1. Voltage over time transition of button release.</div>
	  ...
	  it completes a circuit, letting the chipKIT board read a logic level <code>HIGH</code> (3.3V or 5V) on the corresponding
	  input pin.  You can see that the button has been pressed and is in a <code>HIGH</code>  voltage state.  Once the button
	  ...
	  the voltage level of the signal could momentarily drop below the threshold level needed for detecting a logic level <code>HIGH</code>
  ...
  interval of time apart. An output signal (the debounced version of the input signal) is driven <code>HIGH</code> or <code>LOW</code> based on receiving
  ...
  <code>HIGH</code> (meaning the signal currently is <code>HIGH</code>, and 1/10th of a second ago the signal was also <code>HIGH</code>), it is safe to say the signal
  is in a <code>HIGH</code> state. (Likewise <code>LOW</code> if two consecutive <code>LOW</code> samples are received). In general practice only two samples are ever needed
  to accurately debounce a signal (given that the period between samples is longer than the average period of noise caused by button bounce).
    ...
    <td><span class="TB-BODY"><br>
     ...
     the output signal is the one in blue). Notice that the debounced output signal remains <code>LOW</code> until time T4.
     In this example, the point T4 and its previous point T3 are compared, and since both points are <code>HIGH</code>, the output
     signal is driven <code>HIGH</code>. (Note that the output signal only changes on consecutive samples, so if samples differs,
...
If the previous button state is <code>LOW</code>, and the current state is <code>HIGH</code> then this indicates a rising edge transition (i.e. the signal starting to assert <code>HIGH</code>).
...
<br><br>
    ...
    of debounce if two sample points are both <code>HIGH</code>, or <code>LOW</code>, we drive the signal accordingly.
    <br><br>

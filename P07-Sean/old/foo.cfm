...
<br>
    ...
    <td><span class="TB-BODY">
     ...
     signal as being perfectly crisp and instantaneous transitions.
  ...
  (the debounced version of the signal) HIGH or LOW if consecutive samples are identical. For instance if two samples are both HIGH, and the time
  between them is much longer than the average duration of noise introduced by a bounce, then it is safe to say the signal is in a HIGH state.
  (Likewise the output signal is asserted LOW if two samples are taken that are both LOW).  In general practice only two samples are ever needed
...
<br><br>
    ...
    <br><br>
                <cf_box color="white" style="width:85%">

<html>
	...
	<table class="TBLAYOUT">
		<tr>
			<td>
				<cf_box color="putty">
				<span class="TB-H2">Step 1: Setting up theCircuit</span><br><br>
				<span class="TB-BODY"> 	
					<ol><br>
				...
				<span class="TB-BODY">
				With the circuit built we will now write some simple code to control the color of the LED. Remember the value range for a PWM pin is 0 to 255. In the code below the PWM pins that control red and blue are set to 200 using the <span class="TB-BLACK">setColorBrightness</span><span class="TB-BLACK">()</span> function. This will make the tricolor LED purple.
				...
				<br>
				The function to <span class="TB-BLACK">setColorBrightness</span><span class="TB-BLACK">()</span> may seem unnecessary, since the color can be changed directly with <span class="TB-KEYWORD2">analogWrite</span><span class="TB-BLACK">()</span>. While this is true, bear in mind that it is easy to mix up what pin number corresponds to the color we want to change. Using the <span class="TB-KEYWORD1">if</span> <span class="TB-KEYWORD1">else</span> tree inside of the <span class="TB-BLACK">setColorBrightness</span><span class="TB-BLACK">()</span> function  allows us to modify color values using intuitive color abbreviations, 'r', 'g' and 'b'. Furthermore the <span class="TB-KEYWORD1">if</span> <span class="TB-KEYWORD1">else</span> tree provides a good point of comparison for introduction of <span class="TB-KEYWORD1">switch</span> statements, which will be covered in the next section.
				</span>
				</cf_box>
			</td>
		</tr>
		...
		<tr>
			<td>
				<span class="TB-H1">Switch Statements</span><br><br>
					<span class="TB-BODY">
					<span class="TB-KEYWORD1">switch</span> statements control the flow of a program and are very similar to <span class="TB-KEYWORD1">if</span> <span class="TB-KEYWORD1">else</span> trees.  They compare a variable to a constant <span class="TB-KEYWORD1">case</span> value. If the variable matches the <span class="TB-KEYWORD1">case</span> value, the code contained within the particular <span class="TB-KEYWORD1">case</span> is executed. Otherwise the <span class="TB-KEYWORD1">case</span> is skipped. In the event none of the cases match the default <span class="TB-KEYWORD1">case</span> will be executed. The default <span class="TB-KEYWORD1">case</span> is optional and not needed for the <span class="TB-KEYWORD1">switch</span> statement to function properly. The following is an example of a <span class="TB-KEYWORD1">switch</span> statement.
					</span>
					...
					<br>
					<span class="TB-BODY">
					In the example the <span class="TB-KEYWORD1">case</span> values were int and char types. A <span class="TB-KEYWORD1">case</span> value can be any constant value which can be interpreted as an integer value. This includes the boolean constants <span class="TB-LITERAL1">HIGH</span>, <span class="TB-LITERAL1">LOW</span>, <span class="TB-LITERAL1">INPUT</span>, <span class="TB-LITERAL1">OUTPUT</span>, <span class="TB-KEYWORD1">true</span>, and <span class="TB-KEYWORD1">false</span>. Some data types that cannot be used as a <span class="TB-KEYWORD1">case</span> values are doubles, floats, strings and arrays. On top of being careful with the <span class="TB-KEYWORD1">case</span>'s data type, it is also important to be attentive about including <span class="TB-KEYWORD1">break</span> lines at the end of every <span class="TB-KEYWORD1">case</span>. If you forget to add a <span class="TB-KEYWORD1">break</span> in a specific <span class="TB-KEYWORD1">case</span>, the <span class="TB-KEYWORD1">switch</span> statement will execute that <span class="TB-KEYWORD1">case</span> and all cases below it until it sees a <span class="TB-KEYWORD1">break</span> line. 
				
					<br><br>
				
					So far <span class="TB-KEYWORD1">switch</span> statements and <span class="TB-KEYWORD1">if</span> <span class="TB-KEYWORD1">else</span> trees appear to do the same thing. While this is true, there are distinct advantages and disadvantages when using <span class="TB-KEYWORD1"> switch</span> statements. We know that <span class="TB-KEYWORD1">switch</span> statements have limited use because they cannot compare dynamic variables and are restricted to specific data types. On the other hand <span class="TB-KEYWORD1">switch</span> statements compare only one <span class="TB-KEYWORD1">case</span> making them very efficient. Figures 3 and 4 illustrate the differences in efficiency between <span class="TB-KEYWORD1">if</span> <span class="TB-KEYWORD1">else</span> trees and <span class="TB-KEYWORD1">switch</span> statements.
					...
					The <span class="TB-KEYWORD1">if</span> <span class="TB-KEYWORD1">else</span> tree must evaluate every if statement before and the targeted <span class="TB-KEYWORD1">case</span>. The <span class="TB-KEYWORD1">switch</span> statement, on the other hand, jumps directly to the targeted <span class="TB-KEYWORD1">case</span> and then exits once it is finished. This saves a lots of time in situations with a large number of cases. Furthermore, the efficiency of the <span class="TB-KEYWORD1">switch</span> statement is multiplied inside of the loop since only one <span class="TB-KEYWORD1">case</span> is compared every <!--- every not ever ---> iteration. This will come in handy
					when writing the code for a basic color sweep.         
				</span>
			</td>
		</tr>
	</table>
	...
	<table class="TBLAYOUT" >
		<tr>
			<td>
				...
				Similar to our Basic Color Control code we will use a single function, <span class="TB-BLACK">changeColorBrightness</span><span class="TB-BLACK">()</span>, to control which color is being modified. The main difference here is we have replaced the <span class="TB-KEYWORD1">if</span> <span class="TB-KEYWORD1">else</span> tree with a <span class="TB-KEYWORD1">switch</span> statement and put it inside of a while loop.
	...
	<table class="TBLAYOUT">
		<tr>
			<td>
				...
				In order to select a random color combination a random number must be generated. This can be done using the <span class="TB-KEYWORD2">random</span> <span class="TB-BLACK">()</span> function. The <span class="TB-KEYWORD2">random</span><span class="TB-BLACK">()</span> function can  be used one of two ways. The first way is entering a max value as the only argument. This will return a random integer from 0 to the max minus one. The second way is entering a max and min value as the arguments. This will return a random integer from min to max minus one.
				<br><br>
				For example: 
	...
	<table class="TBLAYOUT" >
		<tr>
			<td>
				...
				Considering there are seven combinations we want to randomly select one each time. This can be achieved by selecting each combination with a <span class="TB-KEYWORD1">switch</span> statement using the <span class="TB-KEYWORD2">random</span> <span class="TB-BLACK">()</span> function to select the <span class="TB-KEYWORD1">case</span>. Inside of each <span class="TB-KEYWORD1">case</span> we want to set the colors to a random brightness.  This will ensure there is a chance for every possible color to be generated. The code for such an algorithm is shown inside of the function <span class="TB-BLACK">randomLEDColorCombo</span><span class="TB-BLACK">()</span>.
				...
				Uploading the sketch to your chipKIT should make the tricolor LED randomly jump to new colors. The time between each jump is determined by the delay inside of the <span class="TB-KEYWORD3">loop</span><span class="TB-BLACK">()</span> function. You may notice some of the colors appear very dim. This is because inside of each <span class="TB-KEYWORD1">case</span>, random(256) has returned a value near 0. You can try experimenting with different min values for the random function to prevent dim colors. You can also specifically tweak the min values in cases 3 through 6. Currently these cases have the possibility of generating a single primary color like red, assuming the other colors in the <span class="TB-KEYWORD1">case</span> are randomly assigned a value near zero. 
				</span>
				...
				Each  sequence will be implemented as a <span class="TB-KEYWORD1">switch</span> statement <span class="TB-KEYWORD1">case</span>. The <span class="TB-KEYWORD1">case</span> will be selected by using the <span class="TB-KEYWORD2">random</span><span class="TB-BLACK">()</span> function.  Inside of each <span class="TB-KEYWORD1">case</span> we will use the <span class="TB-BLACK">changeColorBrightness</span> <span class="TB-BLACK">()</span> function we wrote in the color sweeps section to fade the colors in or out. The code needed to implement this is shown below inside of the <span class="TB-BLACK">randomColorSweep</span><span class="TB-BLACK">()</span> function.
				...
				Once this code is uploaded the tricolor LED should randomly sweep through different color sequences. The sequence's fade time can be tweaked by changing a variable inside of the <span class="TB-BLACK">randomColorSweep</span><span class="TB-BLACK">()</span> function. By default it delays five milliseconds between each incrimination of a colors brightness. This is done using <span class="TB-KEYWORD2">delay</span><span class="TB-BLACK">()</span> called within <span class="TB-BLACK">changeColorBrightness</span> <span class="TB-BLACK">()</span>. If you want to delay for periods less than a millisecond you can replace the <span class="TB-KEYWORD2">delay</span><span class="TB-BLACK">()</span> line with <span class="TB-KEYWORD2">delayMicroseconds</span> <span class="TB-BLACK">(fadeTime)</span> for greater precision. Remember that 1 millisecond is equal to 1000 microseconds, so using delay times less than 300, with <span class="TB-KEYWORD2">delayMicroseconds</span><span class="TB-BLACK">()</span>, will make the LED flash rather than fade.
				</span>
				</cf_box>
			</td>
		</tr>
